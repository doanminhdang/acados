/*    /home/dang/acados/acados/sim/sim_erk_integrator.c
...
void sim_erk(const sim_in *in, sim_out *out, void *mem, void *work_) {
    ...
    int NSTEPS = in->nSteps;
    int NF = in->nsens_forw;
    sim_erk_workspace *work = (sim_erk_workspace*) work_;
    if (!in->sens_forw) {
        NF = 0;
    }
    int nhess = (int)(NF+1)*(real_t)NF/2.0;
...
void sim_erk_create_opts(const int num_stages, sim_RK_opts *opts) {
    if ( num_stages == 1 ) {
