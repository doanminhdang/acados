...
#ifdef DEBUG
static void print_states_controls(real_t *w, int N) {
    printf("node\tx\t\t\t\tu\n");
    for (int i = 0; i < N; i++) {
...
static void shift_states(real_t *w, real_t *x_end, int N) {
    int i,j;
    for ( i = 0; i < N; i++) {
        for ( j = 0; j < NX; j++) w[i*(NX+NU)+j] = w[(i+1)*(NX+NU)+j];
    }
    for ( j = 0; j < NX; j++) w[N*(NX+NU)+j] = x_end[j];
}

static void shift_controls(real_t *w, real_t *u_end, int N) {
    int i,j;
    for ( i = 0; i < N-1; i++) {
        for ( j = 0; j < NU; j++) w[i*(NX+NU)+NX+j] = w[(i+1)*(NX+NU)+NX+j];
    }
    for ( j = 0; j < NU; j++) w[(N-1)*(NX+NU)+NX+j] = u_end[j];
}

// Simple SQP example for acados
int main() {
//  while (true) {
    int i,j,k,iter,sqp_iter;
    // Problem data
    int   N                   = NN;
    real_t  x0[NX]              = {0.5, 0};
    ...
    for ( i = 0; i < NX; i++) w[i] = x0[i];
    for ( i = 0; i < NX; i++) Q[i*(NX+1)] = 1.0;
    for (i = 0; i < NU; i++) R[i*(NU+1)] = 0.05;
    ...
    for ( i = 0; i < NX*(NX+NU); i++) sim_in.S_forw[i] = 0.0;
    for ( i = 0; i < NX; i++) sim_in.S_forw[i*(NX+1)] = 1.0;
    ...
    for ( iter = 0; iter < max_iters; iter++) {
        // printf("\n------ ITERATION %d ------\n", iter);
        acado_tic(&timer);
        for ( sqp_iter = 0; sqp_iter < max_sqp_iters; sqp_iter++) {
            for ( i = 0; i < N; i++) {
                // Pass state and control to integrator
                for (j = 0; j < NX; j++) sim_in.x[j] = w[i*(NX+NU)+j];
                for ( j = 0; j < NU; j++) sim_in.u[j] = w[i*(NX+NU)+NX+j];
                ...
                for ( j = 0; j < NX; j++) {
                    ...
                    for ( k = 0; k < NX; k++) pA[i][j*NX+k] = sim_out.S_forw[j*(NX)+k];
                }
                for ( j = 0; j < NU; j++)
                    for ( k = 0; k < NX; k++) pB[i][j*NX+k] = sim_out.S_forw[NX*NX + NX*j+k];
            }
            for ( j = 0; j < NX; j++) {
                px0[0][j] = (x0[j]-w[j]);
            }
            ...
            for ( i = 0; i < N; i++) {
                for ( j = 0; j < NX; j++) w[i*(NX+NU)+j] += qp_out.x[i][j];
                for ( j = 0; j < NU; j++) w[i*(NX+NU)+NX+j] += qp_out.u[i][j];
            }
            for ( j = 0; j < NX; j++) w[N*(NX+NU)+j] += qp_out.x[N][j];
        }
        for ( i = 0; i < NX; i++) x0[i] = w[NX+NU+i];
